...
async def delete_workout_program(session: AsyncSession, program_id: int) -> bool:
    """Видаляє програму тренувань."""
    try:
...
async def get_workout_stats(session: AsyncSession, client_id: int) -> Dict[str, Any]:
    """Отримує статистику тренувань клієнта."""
    try:
        # Загальна кількість тренувань
        total_stmt = select(func.count(WorkoutSession.id)).where(
            WorkoutSession.client_id == client_id,
            WorkoutSession.completed == True
        )
        ...
        week_stmt = select(func.count(WorkoutSession.id)).where(
            WorkoutSession.client_id == client_id,
            WorkoutSession.completed == True,
            WorkoutSession.date >= week_ago
        )
        week_count = (await session.execute(week_stmt)).scalar() or 0
        
        # Останнє тренування
        last_stmt = select(WorkoutSession).where(
            WorkoutSession.client_id == client_id,
            WorkoutSession.completed == True
        ).order_by(WorkoutSession.date.desc()).limit(1)
        last = (await session.execute(last_stmt)).scalar_one_or_none()
        
        has_active = await get_active_session(session, client_id) is not None
