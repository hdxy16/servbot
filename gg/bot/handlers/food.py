...
class ClientFoodCB(CallbackData, prefix="cl_f"):
    action: str
    category: str = ""
    product_id: int = 0
...
def products_list_kb(products: list, category: str, show_search: bool = True) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for prod in products:
        unit_text = f"({prod.portion_size:g} {prod.unit})"
        builder.button(
    ...
    if show_search:
        builder.row(InlineKeyboardBuilder().button(text="🔍 Пошук / Ввести вручну", callback_data=ClientFoodCB(action="search", category=category).pack()).buttons[0])
    
    builder.row(InlineKeyboardBuilder().button(text="🔙 Скасувати", callback_data=ClientFoodCB(action="main").pack()).buttons[0])
    return builder.as_markup()